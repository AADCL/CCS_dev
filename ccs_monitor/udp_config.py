...
class UdpTelemetryConfig:
    ...
    def descriptor_hash(self) -> str:
        canonical = [item.canonical() for item in sorted(self.descriptors, key=lambda value: value.name)]
...
def load_udp_config(path: Path = DEFAULT_UDP_CONFIG_PATH) -> UdpTelemetryConfig:
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != 1:
        raise UdpConfigError("UDP 配置 schema_version 必须为 1")
    ...
    descriptors: list[TelemetryDescriptor] = []
    names: set[str] = set()
    for index, raw in enumerate(raw_descriptors):
        if not isinstance(raw, dict):
            raise UdpConfigError(f"descriptors[{index}] 必须是对象")
        name = raw.get("name")
        display_name = raw.get("display_name")
        data_type = raw.get("type")
        level = raw.get("level")
        if not isinstance(name, str) or not name.strip() or name in names:
            raise UdpConfigError(f"descriptors[{index}] 名称为空或重复")
        if not isinstance(display_name, str) or not display_name.strip():
            raise UdpConfigError(f"descriptors[{index}] display_name 无效")
        if data_type not in ALLOWED_TYPES or level not in LEVEL_RATES:
            raise UdpConfigError(f"descriptors[{index}] 类型或等级无效")
        names.add(name)
        descriptors.append(TelemetryDescriptor(name, display_name, data_type, level))
    try:
