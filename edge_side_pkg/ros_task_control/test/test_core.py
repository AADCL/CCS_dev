...
from ros_task_control.config import ConfigError, load_config
from ros_task_control.protocol import ProtocolError, decode
from ros_task_control.storage import StorageError, TrajectoryStore, decode_trajectory


PACKAGE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
TASK_CONFIG = os.path.join(PACKAGE, "config", "task_control.yaml")
DEVICE_CONFIG = os.path.join(os.path.dirname(PACKAGE), "edge_device_config", "config", "device.yaml")


def trajectory(device_id="UAV_001", task_name="园区 & 巡检"):
    return {
        "schema_version": 1, "task_id": "task/../1", "task_name": task_name,
...
class CoreTests(unittest.TestCase):
    ...
    def test_protocol_rejects_wrong_protocol(self):
        raw = {"schema_version": 1, "protocol_id": "wrong", "task_id": "t", "subtask_id": "s",
