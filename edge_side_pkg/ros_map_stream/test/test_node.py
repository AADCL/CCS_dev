...
from ros_map_stream.config import load_config
from ros_map_stream.node import RosMapStreamNode
from ros_map_stream.protocol import encode_envelope


PACKAGE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
MAPPING = os.path.join(PACKAGE, "config", "mapping.yaml")
DEVICE = os.path.join(os.path.dirname(PACKAGE), "edge_device_config", "config", "device.yaml")
...
class NodeTests(unittest.TestCase):
    ...
    def test_synchronized_cloud_callback_sends_chunk_frame(self):
        ...
        with patch("ros_map_stream.node.extract_pointcloud2", return_value=np.asarray([[1, 0, 0], [2, 0, 0]], dtype=np.float32)):
