...
<!-- ROS_MAP_STREAM_VERSION: 0.1.0 -->

版本：`v0.1.0`。

`ros_map_stream` 是 ROS Melodic/Python 3.6.9 端侧实时建图数据包。节点在 standby 状态监听地面站 UDP 14561 指令；建图期间同步 PointCloud2 与位姿，以 `ccs-map-stream-v1` 向指令指定且经配置校验的 UDP 14562 上传分片点云、同步位姿和静态外参。
...
可覆盖 `mapping_config_file` 与 `device_config_file` launch 参数。设备 ID/IP 始终来自 `edge_device_config/config/device.yaml`。
