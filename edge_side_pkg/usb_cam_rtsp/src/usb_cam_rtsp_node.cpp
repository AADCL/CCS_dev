...
class UsbCamRtspNode {
 public:
  UsbCamRtspNode()
    ...
    ROS_INFO_STREAM("usb_cam_rtsp ready device_id=" << device_id_ << " device_ip="
 ...
 private:
  ...
  void imageCallback(const sensor_msgs::ImageConstPtr& message) {
    try {
      ...
      ROS_INFO_ONCE("usb_cam_rtsp received first camera frame");
    } catch (const cv_bridge::Exception& error) {
      ROS_ERROR_THROTTLE(5.0, "usb_cam_rtsp image conversion failed: %s", error.what());
    }
  }
  ...
  void watchdogCallback(const ros::WallTimerEvent&) {
    std::lock_guard<std::mutex> lock(frame_mutex_);
    if (!received_frame_) {
      ROS_WARN_THROTTLE(5.0, "usb_cam_rtsp is waiting for camera frames on %s", image_topic_.c_str());
      return;
    }
    if ((ros::WallTime::now() - last_frame_time_).toSec() > frame_timeout_seconds_) {
      ROS_ERROR_THROTTLE(5.0, "usb_cam_rtsp camera frames have stopped");
    }
  }
  ...
  static void mediaConfigure(GstRTSPMediaFactory*, GstRTSPMedia* media, gpointer user_data) {
    ...
    if (source != nullptr) {
      ...
      ROS_ERROR("usb_cam_rtsp could not locate GStreamer appsrc");
    }
  ...
  static void needData(GstElement* appsrc, guint, gpointer user_data) {
    ...
    if (result != GST_FLOW_OK) {
      ROS_WARN_THROTTLE(5.0, "usb_cam_rtsp appsrc push returned %d", result);
    }
  }

  static void clientConnected(GstRTSPServer*, GstRTSPClient* client, gpointer user_data) {
    UsbCamRtspNode* node = static_cast<UsbCamRtspNode*>(user_data);
    ROS_INFO_STREAM("usb_cam_rtsp RTSP client connected for device " << node->device_id_);
  ...
  static void clientClosed(GstRTSPClient*, gpointer user_data) {
    UsbCamRtspNode* node = static_cast<UsbCamRtspNode*>(user_data);
    ROS_INFO_STREAM("usb_cam_rtsp RTSP client disconnected for device " << node->device_id_);
  }
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "usb_cam_rtsp");
  try {
    UsbCamRtspNode node;
    ros::spin();
    return 0;
  } catch (const std::exception& error) {
    ROS_FATAL("usb_cam_rtsp startup failed: %s", error.what());
    return 1;
  }
}
