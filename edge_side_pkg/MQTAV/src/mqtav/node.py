...
def default_device_config_path():
    source_path = Path(__file__).resolve().parents[3] / "edge_device_config" / "config" / "device.yaml"
    if source_path.is_file():
        return source_path
    try:
        import rospkg

        return Path(rospkg.RosPack().get_path("edge_device_config")) / "config" / "device.yaml"
    except (ImportError, OSError, RuntimeError):
        return source_path
    try:
        import rospkg

        return Path(rospkg.RosPack().get_path("mqtav")) / "config" / "config.yaml"
    except (ImportError, OSError, RuntimeError):
        return source_path
