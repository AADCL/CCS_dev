...
编辑 `config/config.yaml` 和共享的 `edge_device_config/config/device.yaml` 后再启动节点。以下字段均为必填，除 `ros.mission` 可保持 `enabled: false` 外：
...
roslaunch mqtav mqtav.launch \
  ...
  device_config_file:="$(rospack find edge_device_config)/config/device.yaml"
```
