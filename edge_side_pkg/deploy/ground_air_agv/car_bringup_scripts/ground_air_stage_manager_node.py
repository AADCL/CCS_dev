#!/usr/bin/env python3
"""Own mapping/relocalization roslaunch groups without changing launch files."""
...
from system_stage_core import BASE, MAPPING, RELOCALIZATION, StageError
from system_stage_core import analyze_active_nodes, build_stage_commands, normalize_request
...
class RosProcessBackend:
    ...
    def wait_primary(self, stage, process, timeout):
        ...
        def ready():
            return required.issubset(set(rosnode.get_node_names()))
    ...
    def stop(self, process):
        ...
        if process.poll() is None:
            try:
                ...
                raise RuntimeError("managed process group {} did not stop cleanly".format(process.pid))
...
class StageManagerNode:
    ...
    def _ccs_owner(request):
        # Caller identity is an ownership guard, not a security/authentication boundary.
        caller = getattr(request, "_connection_header", {}).get("callerid", "")
        return caller if caller.startswith("/ccs_mapping_stage_") else ""
    ...
    def _handle_stage(self, request):
        with self._lock:
            ...
            if result.success:
                rospy.loginfo("system stage %d: %s", result.active_stage, result.message)
            else:
                rospy.logerr("system stage request failed: %s", result.message)
            return SetSystemStageResponse(
    ...
    def _monitor_children(self, _event):
        with self._lock:
            ...
            message = "managed stage exited unexpectedly (codes: {}); base stage active".format(
                codes
            )
...
def main():
    ...
    StageManagerNode()
    rospy.loginfo("robot-side ground-air stage manager is ready")
    rospy.spin()
