#!/usr/bin/env python3
"""ROS entry point for the Scout Mini navigation task adapter."""
...
def main():
    rospy.init_node("scout_navigation_task_adapter")
    package = "epgeneral_task_control"
    ...
    required = ("active_map_state_file", "navigation_launch_package", "navigation_launch_file",
                "navigation_map_root", "navigation_map_yaml", "odom_topic",
                "navigation_action", "zero_velocity_topic")
    missing = [key for key in required if not adapter_config.get(key)]
    if missing:
        raise rospy.ROSInitException("Scout adapter config is missing: %s" % ", ".join(missing))
    merged = dict(config)
    merged.update(adapter_config)
    adapter = ScoutNavigationAdapter(rospy, merged, TaskExecutionCommand, TaskExecutionFeedback)
    adapter.start()
    rospy.loginfo("Scout navigation task adapter started")
    rospy.spin()
