...
版本：`v0.3.1`。Scout Mini 通过 `/fastlio_odom`、实时 `map<-odom` TF 和 `/move_base/goal` 执行有序地图航点；任务目录默认位于 `~/ccs_edge_ws/mission/`。

节点监听 UDP 14563，向地面站 UDP 14564 发送 ACK、1 Hz 心跳、任务状态和航点进度。Scout 适配器在执行时启动 `scout_navigation navigation_teb.launch map_name:=<map_id>`，终止或急停时取消目标、发布 `/cmd_vel` 零速度并停止导航进程。

## 状态与数据

- 接收状态：`no_task -> receiving -> received -> ready`，错误进入 `failed`，急停依次进入 `emergency_stop -> no_task`。
...
- Scout 默认要求导航在 25 秒内且早于统一启动时刻就绪，单航点超时 300 秒；启动等待期间每秒反馈 `scheduled`。
- 执行前必须同时存在有效的实时 `/fastlio_odom` 和 `map<-odom` TF；仅有历史 `relocalization.json` 不允许启动导航。错误码包括 `LOCALIZATION_UNAVAILABLE`、`MAP_FRAME_MISMATCH` 和 `NAVIGATION_STARTUP_TIMEOUT`。
