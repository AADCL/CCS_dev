...
节点监听 UDP 14563，向地面站 UDP 14564 发送 ACK、1 Hz 心跳、任务状态和航点进度。任务包不会解锁飞控或直接调用 MAVROS；设备控制节点通过 `/epgeneral_task_control/execution_command` 和 `/epgeneral_task_control/execution_feedback` 接入。

## 状态与数据

- 接收状态：`standby -> receiving -> ready`，错误清理后回到已有任务的 ready 或 standby。
- 执行状态：`scheduling -> scheduled -> running -> completed/stopped/failed`。
- 轨迹网络内容保持 zlib JSON；提交后写入 `~/.ros/epgeneral_task_control/tasks/<ID哈希>/trajectory.xml`。
