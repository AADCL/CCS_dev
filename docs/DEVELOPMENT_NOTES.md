...
- 大屏列表继续复用 QListWidgetItem 和原 UserRole 设备 ID；独立绘制代理读取快照展示图标、电池和文本。图标按路径及文件时间缓存，主题回退图标分别缓存；遥测更新不重建选中项。
- 控制台分组的 sizeHint 根据当前字体测量标签并预留控件宽度，避免在挂接父级样式前固定标签尺寸。样式、状态徽标和电池组件限定于大屏。
- 大屏复用 PointCloudViewer 的 set_task_paths / set_task_conflicts，直接使用任务地图 XYZ。viewer.clear() 不负责任务叠加缓存，因此大屏在地图加载前显式清空路径和冲突，并在选择、仓库更新、加载结束与异步恢复后同步当前任务。
- 冲突缓存键包含任务 ID、设备路径、速度、延迟和安全参数；遥测循环不计算冲突，运行回执不触发相同任务定义的重复计算。地图不匹配只提示，不隐式激活地图或调整任务启停条件。
- 日夜 SVG 操作图标位于 icons/app_icons，继续由现有资源加载器和分发脚本处理；依赖锁仅更新本包版本。复现入口与截图见 [本次增量验证](validation/dashboard-polish/README.md)。
