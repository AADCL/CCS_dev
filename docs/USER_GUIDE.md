...
多设备任务页新增 **同步下发**。先为每台任务设备编辑完整路线，再点击同步下发；平台会先校验全部草稿并保存本轮版本，然后向全部设备发送子任务。任一草稿无效或设备离线时，不发送本轮任务。下发过程中按钮禁用，避免重复提交。

设备卡片分别显示“下发中”“已接收待就绪”“就绪”或“失败”，并显示就绪数量。只有全部设备接收当前版本成功、端侧导航返回 READY、没有未下发修改、设备离线或执行冲突时，才能点击 **开始主任务**。开始操作沿用统一 UTC 启动时间，不再重新保存或增加 revision。编辑一台设备只使该设备的就绪状态失效；其他设备的草稿和成功状态保留。部分设备失败时，可从该设备原有下发入口重试。

重新打开任务会重新协商端侧状态，磁盘中的旧 READY 不作为启动依据。导航准备默认等待 60 秒；可通过 `config/task_system.json` 的 `navigation_ready_timeout_seconds` 调整。传输拒绝、超时或导航失败会显示原因，并释放本轮请求。

任务地图与指控大屏各有独立的 **轨迹** 开关，默认开启，并记住各自偏好。任务页显示该任务设备，大屏显示当前地图的有效设备轨迹。隐藏轨迹、切换页面或选中设备均不停止采集。轨迹仅接收具有有效地图坐标变换的位姿；重连、UDP 会话或地图变换改变时断开线段，避免误连。

轨迹保存在应用数据目录 `data/trajectories/`，后台每秒刷新。短暂断联不足 120 秒时续接本次连接；断联满 120 秒后记录失效并清理文件，下一次连接建立新记录。重启时恢复未过期记录。轨迹图采用全程抽稀轮廓；文件保留所有已采集的有效地图位姿，2 厘米的显示去重不影响文件记录。磁盘错误显示提示；持续写入失败达到缓存上限后可能丢失新点，应恢复磁盘空间后重新检查。

电池历史按分钟聚合，保留两天，每分钟后台批量保存并在退出时刷新。任务事件日志继续保存完整历史，界面按需读取最近 500 条。切换离开地图页面会释放静态渲染数据，再进入时后台读取文件并恢复视角。
...
v0.24.1 的在线设备卡片显示设备类型中已上传的图标；图标缺失时使用通用设备图标。电池图形与百分比共用原电量数据：未知显示 `--`，低于 25% 显示低电量色；右栏保留一位小数。右栏的名称和编号继续沿用设备身份色，在线、离线与健康状态同时提供文字和状态色。

底部地图、任务选择框及“当前设备”摘要按组排列，窗口变窄时换行。当前设备仍通过左栏选择；长名称省略时悬停可查看全文。顶部导航图标、居中标题、视角操作和全屏 / ESC 使用方式不变。

选择可执行任务后，当前地图上显示该任务全部设备的规划路线、任务点及冲突点。任务属于其他地图时，控制台持续提示“任务属于〈地图名〉，请切换地图”；通过原地图选择器切换到该地图即可显示。切换任务、修改已保存路线和删除任务后自动更新图层。该显示操作不会开始或终止任务。

任务路线与实时连接轨迹独立：**轨迹** 开关控制连接轨迹；点云 / PGM / 叠加控制底图。地图底部高度滑块向右移动时隐藏更高点云，不过滤任务点。地图部分图层加载成功时仍可显示匹配任务；全部加载失败时清除任务图层并保留错误提示。
