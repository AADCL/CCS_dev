...
指控平台配套七个公共端侧包：`epgeneral_device_config` v0.1.1、`epgeneral_mqtav` v0.4.1、`epgeneral_udp_telemetry` v0.3.1、`epgeneral_video_srt` v0.1.1、`epgeneral_map_stream` v0.13.2、`epgeneral_task_control` v0.5.0 和 `epgeneral_relocalization` v0.3.0。Ground-Air profile 另部署 `epgeneral_ground_air_control` v0.2.0。MQTT schema 1.0、SRT 与 UDP wire schema 保持兼容。
