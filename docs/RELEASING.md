...
本次新增 `data/trajectories/` 连接轨迹目录，随现有应用数据保留策略一起保留。任务文件无需迁移；旧 `task_system.json` 缺少 `navigation_ready_timeout_seconds` 时采用 60 秒默认值。产品版本元数据与 uv.lock 已同步为 0.24.0，ROS 包和端侧协议版本不变。

本次源码验证、实际依赖环境、原始性能数据及未验证项见 [发布验证记录](RELEASE_VALIDATION.md)。若构建正式安装包，仍需按下面流程使用锁定依赖重新验证显卡绘制、安装升级和真实设备；本次 Qt/VisPy CPU 回放不代替这些验收。
