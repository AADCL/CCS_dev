...
- **系统总览**：以语义图标、文字和数值共同展示在线/离线设备、地图和任务执行记录，并集中展示 NTP、MQTT、UDP、建图、任务、FFmpeg/SRT 及本地仓储状态。
...
- **设备重定位**：完整地图可下发至 Scout；同一地图仅允许一台设备进入启动、选点或解算流程，地图中心十字星与设备 ID 绑定后下发初始位姿。成功的 `map <- odom` 绑定持久化并用于各地图视图的在线设备标记。Go2 当前显示为不支持。
- **任务系统**：通过共享地图图层控件进行 PCD/PGM 选点，支持多设备子任务、XYZ 编辑、轨迹冲突检查、持久化、UDP 14563/14564 下发与同步执行；任务日志统一展示审计和执行事件，可自动滚动并清除当前视图。
- **指控大屏**：集中展示在线设备、三维地图、本地 odom 位置/姿态趋势和任务状态；趋势窗口从 5 秒动态扩展至 60 秒，列表、状态、地图、轨迹和曲线使用统一设备颜色。设备始终置顶于地图图层，支持全屏、Esc 安全退出及面板折叠。
- **状态图标与品牌**：首页全部 10 张子系统卡、设备列表统计卡及 MQTT 状态均保留图标和文字；日夜换肤不重置筛选、编辑选择或运行状态。应用 SVG 统一位于 `icons/app_icons/`，地图使用 `mapStorage`，任务仓储使用 `taskStorage`；顶栏版本号右侧显示实验室 Logo 与 AADCL。
- **主题化操作图标**：导航、返回、下发、主题切换和展开/收起控件使用 `icons/app_icons/` 中的日间/夜间 SVG，换肤时即时更新且不改变页面状态。
...
- 左侧显示 MQTT 在线设备，中间显示 PCD/PGM/叠加地图，右侧显示由同一份本地 odom 位姿生成的状态和位置/姿态趋势；没有有效 odom 时两组数据同时显示无数据。
